...
switch MODE
    ...
    case 'Caltech' % Caltech dataset
        ...
        %imshow(C(1,:)); drawnow; disp(size(C));
        
        disp('Encoding Images...')
        data_train = zeros(150,257);
        % Vector Quantisation
        for c = 1:length(classList)
            for i = 1:length(imgIdx_tr)
                Idx = knnsearch(C,single(desc_tr{c,i}.'));
                for cnt = 1:numBins
                    bow = zeros(size(numBins));
                    bow(cnt) = sum(Idx == cnt);
                end
                data_train(15*(c-1)+i, 1:256) = bow;
                data_train(15*(c-1)+i, 257) = c;
            end
        end
        %disp(size(data_train{1,1}));
        histogram(Idx,256);
...
%         end
        ...
        % write your own codes here
        data_query = zeros(150,257);
        for c = 1:length(classList)
            for i = 1:length(imgIdx_tr)
                Idx = knnsearch(C,single(desc_te{c,i}.'));
                for cnt = 1:numBins
                    bow = zeros(size(numBins));
                    bow(cnt) = sum(Idx == cnt);
                end
                data_query(15*(c-1)+i, 1:256) = bow;
                data_query(15*(c-1)+i, 257) = c;
            end
        end     
    disp('Q1 done!')
